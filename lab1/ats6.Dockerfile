...
RUN yum clean all && yum update -y
RUN wget http://archive.apache.org/dist/trafficserver/trafficserver-6.2.2.tar.bz2
RUN tar -xvf trafficserver-6.2.2.tar.bz2
WORKDIR /tmp/trafficserver-6.2.2
RUN autoreconf -if
